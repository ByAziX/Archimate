...
(function () {
    ...
    $JExcel.new = function (defaultFont) {
        ...
        excel.removeSheet = function (name) {
            // Remove a sheet by name
            var indexToRemove = sheets.sheets.findIndex(sheet => sheet.name === name);
            if (indexToRemove > -1) {
                sheets.sheets.splice(indexToRemove, 1);
                // Update rId and possibly other properties if necessary
                sheets.sheets.forEach((sheet, index) => {
                    sheet.id = index + 1;
                    sheet.rId = "rId" + (3 + index);
                });
            }
        };
